...
def diag_rsl_prm(df_state, df_output):
    ...
    for idx in df_suews.index:
        ...
        #     avdens,
        # ) = sd.atmmoiststab_module.cal_atmmoist(temp_c, press_hpa, avrh, 0.0)
        l_mod_rsl,zh_rsl,lc,beta,zd,z0,elm,scc,f = sd.rsl_module.rsl_cal_prms(
            zh_min,
            z0m,
            zdm,
            stabilitymethod,
        ...
        dict_prm.update({idx.isoformat(): [l_mod_rsl,zh_rsl,lc,beta,zd,z0,elm,scc,f]})

    # post-process results
    df_prm = pd.DataFrame.from_dict(
        dict_prm, orient="index", columns=['l_mod_rsl','zh_rsl','lc','beta','zd','z0','elm','scc','f']
    )
